...
ezra({
  ...
  await repondre("*☢️LUCKY-MD-XFORCE☢️ is sending your message to all groups ,,,💀*...");

  const broadcastMessage = `*🔋 ☢️LUCKY-MD-XFORCE☢️ 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓🔋*\n\n🀄 Message: ${arg.join(" ")}\n\n🗣️ Author: ${nomAuteurMessage}`;
...
ezra({ nomCom: "disap1", categorie: "Group", reaction: '☢️' }, async (chatId, client, context) => {
  handleDisapCommand(chatId, client, context, 86400); // 1 day
});
ezra({ nomCom: "disap7", categorie: 'Group', reaction: '☢️' }, async (chatId, client, context) => {
  handleDisapCommand(chatId, client, context, 604800); // 7 days
});
ezra({ nomCom: "disap90", categorie: 'Group', reaction: '☢️' }, async (chatId, client, context) => {
